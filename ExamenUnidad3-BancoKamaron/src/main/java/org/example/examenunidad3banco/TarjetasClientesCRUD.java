...
public class TarjetasClientesCRUD implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        if (usuario != null) {
            ...
            SolicitudTarjetaCredito solicitud = null;
            for (SolicitudTarjetaCredito solicitudTC : GerenteMaderoCRUD.solicitudesTC) {
    ...
    public void solicitarSimplicity(ActionEvent event) throws IOException {
        ...
        if (evtSolicitarSimplicity.equals(btnSolicitarSimplicity)) {
            if (saldoActual < 50000) {
                ...
                SolicitudTarjetaCredito solicitud = new SolicitudTarjetaCredito(usuario, TipoTarjetaCredito.SIMPLICITY, usuario.getTarjetaDebito().getSaldo(), EstadoTarjetaCredito.EN_ESPERA);

                txtEstadoSimplicity.setText(String.valueOf(EstadoTarjetaCredito.EN_ESPERA));

                GerenteMaderoCRUD.solicitudesTC.add(solicitud);
    ...
    public void solicitarPlatino(ActionEvent event) throws IOException {
        ...
        if (evtSolicitarPlatino.equals(btnSolicitarPlatino)) {
            if (saldoActual < 100000) {
                ...
                SolicitudTarjetaCredito solicitud = new SolicitudTarjetaCredito(usuario, TipoTarjetaCredito.PLATINO, usuario.getTarjetaDebito().getSaldo(), EstadoTarjetaCredito.EN_ESPERA);
    ...
    public void solicitarOro(ActionEvent event) throws IOException {
        ...
        if (evtSolicitarOro.equals(btnSolicitarOro)) {
            if (saldoActual < 200000) {
                ...
                SolicitudTarjetaCredito solicitud = new SolicitudTarjetaCredito(usuario, TipoTarjetaCredito.ORO, usuario.getTarjetaDebito().getSaldo(), EstadoTarjetaCredito.EN_ESPERA);
    ...
    private boolean haySolicitudPendiente(Clientes usuario, TipoTarjetaCredito tipoTarjeta) {
        for (SolicitudTarjetaCredito solicitud : GerenteMaderoCRUD.solicitudesTC) {
