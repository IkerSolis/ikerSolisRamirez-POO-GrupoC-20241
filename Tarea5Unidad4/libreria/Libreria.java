...
public class Libreria {
    ...
    public static void añadirUsuarioJson(Usuario usuario) {

        ArrayList<Usuario> clientes = new ArrayList();
        ArrayList<Usuario> asistentes = new ArrayList();
        ArrayList<Usuario> gerente = new ArrayList();

        if(usuario.getRol() == Rol.CLIENTE) {
            clientes.add(usuario);
            Libreria.usuarios.put(Rol.CLIENTE, clientes);
        } else if (usuario.getRol() == Rol.ASISTENTE) {
            asistentes.add(usuario);
            Libreria.usuarios.put(Rol.ASISTENTE, asistentes);
        } else if (usuario.getRol() == Rol.GERENTE) {
            gerente.add(usuario);
            Libreria.usuarios.put(Rol.GERENTE, gerente);
        }
    ...
    public static void añadirLibroJson(Libro libro) {

        ArrayList<Libro> librosTerror = new ArrayList();
        ArrayList<Libro> librosAccion = new ArrayList();
        ArrayList<Libro> librosComedia = new ArrayList();

        if(libro.getGenero() == Genero.TERROR) {
            librosTerror.add(libro);
            Libreria.libros.put(Genero.TERROR, librosTerror);
        } else if (libro.getGenero() == Genero.ACCION) {
            librosAccion.add(libro);
            Libreria.libros.put(Genero.ACCION, librosAccion);
        } else if (libro.getGenero() == Genero.COMEDIA) {
            librosComedia.add(libro);
